...
const statusConfig: Record<Employee['status'], { bg: string; text: string; label: string }> = {
...
function EmployeeCard({ employee, onSelect }: EmployeeCardProps) {
  ...
  return (
    <div
      ...
      className={`bg-white rounded-xl border border-slate-200 p-5 w-full transition-all duration-200 ${
        onSelect ? 'cursor-pointer hover:shadow-md hover:border-blue-300' : ''
      }`}
    >
      <div className="flex items-center gap-3">
        <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center overflow-hidden text-blue-700 font-semibold text-lg shrink-0">
          {avatarUrl ? (
